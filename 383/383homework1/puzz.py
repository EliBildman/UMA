...
class EightPuzzleBoard:
    ...
    def _create_successor(self, delta_x, delta_y):  # create a successor object (or None if invalid)
        ...
        if (move_x < 0) or (move_x > 2) or (move_y < 0) or (move_y > 2):
            ...
            succ.cost = self.cost + int(self._get_tile(move_x, move_y))
            return succ

    def get_loc(self, num):
        i = self._board.index(str(num))
        x = i % 3
        y = i / 3
        return (x, y)

    def distance_from_goal(self, num):
        x, y = self.get_loc(num)
        target_x, target_y = EightPuzzleBoard(goal_state).get_loc(num)
    ...
    def is_goal(self):
        return self._board == list(goal_state)
